...
// import { Check, ShieldCheck, Clock, History, Search, Sparkles, Award, AlertTriangle, ChevronRight } from "lucide-react";
// import {
//   Dialog,
//   DialogContent,
...
//   const getRiskBadge = (productId: string) => {
//     const risk = returnRiskData[productId];
//     if (!risk) return null;
//     if (risk.riskLevel === "high") {
//       return (
//         <div className="flex items-center gap-1 mt-1.5 bg-red-50 border border-red-200 rounded px-1.5 py-0.5" data-testid={`badge-risk-${productId}`}>
//           <AlertTriangle className="h-3 w-3 text-red-500 shrink-0" />
//           <span className="text-[10px] text-red-700 font-medium">{risk.returnRate}% return rate</span>
//         </div>
//       );
//     }
//     if (risk.riskLevel === "medium") {
//       return (
//         <div className="flex items-center gap-1 mt-1.5 bg-amber-50 border border-amber-200 rounded px-1.5 py-0.5" data-testid={`badge-risk-${productId}`}>
//           <AlertTriangle className="h-3 w-3 text-amber-500 shrink-0" />
//           <span className="text-[10px] text-amber-700 font-medium">{risk.returnRate}% return rate</span>
//         </div>
//       );
//     }
//     return null;
...
//         {/* Return Risk Filter */}
//         <div className="border-t border-gray-200 pt-4">
//           <h3 className="font-bold text-sm mb-3">Predictive Return Risk</h3>
//           <div className="space-y-2">
//             <div className="flex items-center space-x-2">
//               <Checkbox id="risk-low" />
//               <Label htmlFor="risk-low" className="text-sm font-normal text-green-700">Low risk only</Label>
//             </div>
//             <div className="flex items-center space-x-2">
//               <Checkbox id="risk-certified" />
//               <Label htmlFor="risk-certified" className="text-sm font-normal">Certified Refurbished</Label>
//             </div>
//           </div>
//         </div>
//       </aside>
...
//                     <button className="mt-2 w-full bg-primary hover:bg-accent text-black text-[11px] font-medium rounded-full py-1 transition-colors" data-testid={`button-add-rec-${rec.id}`}>
//                       Add to Cart
//                     </button>
//                   </div>
//                 </div>
...
//                   {/* Predictive return risk inline warning */}
//                   {risk && (
...
//                     <Badge variant="outline" className={getConditionColor(selectedProduct.condition)}>
...
//                       </h4>
//                       <p className="text-xs text-gray-900">{selectedProduct.history}</p>
//                     </div>
//                     <div className="flex-1 bg-gray-50 border border-gray-200 rounded-lg p-3">
//                       <h4 className="text-xs font-bold text-gray-700 flex items-center gap-1.5 mb-1">
//                         <Clock className="h-4 w-4 text-gray-500" /> Warranty
//                       </h4>
//                       <p className="text-xs text-gray-900">{selectedProduct.warranty}</p>
//                     </div>
//                   </div>
//                 </div>

//                 <div className="mt-6 pt-4 border-t border-gray-200 space-y-3">
//                   <Button className="w-full bg-accent hover:bg-orange-400 text-black font-bold h-10 rounded-full border border-accent-border" data-testid="button-buy-now-modal">
//                     Buy Now
//                   </Button>
...
//                     </div>
//                     <Button variant="outline" className="h-9 px-4 text-sm font-medium" data-testid="button-submit-offer">Submit Offer</Button>
//                   </div>
//                 </div>
//               </div>
//             </div>
...
export default function Marketplace() {
  ...
  const handleAddToCart = (product: any, isRecommendation = false) => {
    try {
      // 1. Get existing cart items from localStorage (or defaults to empty array)
      const existingCart = JSON.parse(localStorage.getItem("recommerce_cart") || "[]");

      // 2. Format the added item to match the schema expected by Cart.tsx
      const newCartItem = {
        id: `cart-${product.id}-${Date.now()}`, // Generate unique cart line item ID
        title: product.name,
        condition: isRecommendation ? `Recommended - ${product.reason}` : `${product.condition} (${product.score}/10 Health)`,
        price: product.price,
        originalPrice: product.originalPrice,
        image: product.image || "https://images.unsplash.com/photo-1505740420928-5e560c06d30e?w=150&q=80", // fallback image matching placeholder
        co2Saved: isRecommendation ? "9.4 kg" : `${((product.score || 8) * 1.3).toFixed(1)} kg`,
        greenCreditsEarned: isRecommendation ? 30 : Math.round((product.price * 0.005)),
        quantity: 1
      };

      // 3. Append item and update local state database storage
      const updatedCart = [...existingCart, newCartItem];
      localStorage.setItem("recommerce_cart", JSON.stringify(updatedCart));

      // 4. Dispatch a custom window event so that the Header instantly reflects item count changes if needed
      window.dispatchEvent(new Event("storage"));

      // 5. Notify user
      toast.success(`Added "${product.name.substring(0, 25)}..." to your Eco-Cart!`);
    } catch (error) {
                ...
                >
                  ...
                  <div className="p-2.5">
                    ...
                    <button 
                      ...
                      onClick={(e) => {
                        e.stopPropagation(); // prevent modal popup from triggering
                ...
                <CardContent className="p-3 flex flex-col flex-1">
                  ...
                  <div className="mt-auto pt-2">
                    ...
                    <Button
                      className="w-full bg-primary hover:bg-accent text-black font-medium h-8 text-xs rounded-full border border-primary-border"
                      onClick={(e) => {
                        e.stopPropagation(); // prevent modal dialog opening click execution trigger
